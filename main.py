...
import pytesseract
import time
import ImageTextExtractor


class PDFTextExtractor:
    def __init__(self, image_url, column_num):
    ...
    def pdf2jpg(self):
        pages = list()
        if not self.image_url.endswith('.pdf'):
            print("This is not a PDF file")
        else:
            pages = convert_from_path(self.image_url)
        return pages
    ...
    def process_pdf(self):
        ...
        column_functions = {
            1: self.data_to_text_json_1,
            2: self.data_to_text_json_2,
        }

        if self.column_num in column_functions:
            selected_function = column_functions[self.column_num]
            for i, image in enumerate(self.images):
                width, height = image.size
                extracted_data = self.extract_data_from_image(image, 25)
...
if __name__ == "__main__":
    extractor = PDFTextExtractor("./test/Meeting Minute3.pdf", 2)
